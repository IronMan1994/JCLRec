...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, ifDropout):
        """
        :param queries: A 3d tensor with shape of [N, T_q, C_q],   seq_normalized
        :param keys: A 3d tensor with shape of [N, T_k, C_k],      seq
        ...
        # Key Masking
        # torch.sign(tensor)：tensor中整数都为1，负数都为-1,0则为0
        # 使占位编号的项为0，这里的keys是cacu_h中进行占位符mask后的seq，queries是对keys进行标准化后，最后一维相加可判断是否全部为0，如果全部为0，则key_mask为0。
        ...
        # torch.where(condition, x, y)：当condition为真返回x的值，否则返回y的值
        ...
        if ifDropout:
            ...
            matmul_output_dropout = matmul_output_qm
